...
**Recommendation**:
- Remove old methods or mark as deprecated
- Create new methods for each config field:
  - `get_runner_id()`, `set_runner_id()`
  - `get_admin_id()`, `set_admin_id()`
  - `get_active_elo()`, `set_active_elo()`
  - `get_default_rank()`, `set_default_rank()`
- Remove `ConfigFormat` struct entirely
...
**Recommendation**:
- Add migration to copy `users.elo` to `elo` table for all guilds
- Drop `users.elo` column after migration
- Document that ELO is now guild-specific
