...
use crate::{handlers::role::check_role, CommandContext};
...
pub async fn handle_config_command(
    ...
    if !check_role(cc, &Role::Admin).await? {
        let response = CreateInteractionResponse::Message(
            CreateInteractionResponseMessage::new()
                .content("Only session admins can modify the config!")
                .ephemeral(true),
        );
        cc.intax.create_response(&cc.ctx.http, response).await?;
        return Ok(());
...
fn parse_user_mention(mention: &str) -> Result<u64> {
    ...
    if mention.starts_with("<@!") && mention.ends_with('>') {
        Ok(mention[3..mention.len() - 1]
            .to_string()
            .parse::<u64>()
            .unwrap())
    } else if mention.starts_with("<@") && mention.ends_with('>') {
        Ok(mention[2..mention.len() - 1]
            .to_string()
            .parse::<u64>()
            .unwrap())
    } else {
