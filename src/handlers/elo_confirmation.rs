use anyhow::Result;
use serenity::all::{ComponentInteraction, Context, CreateEmbed as CE, CreateInteractionResponse as CIR, CreateInteractionResponseMessage as CIRM, UserId as UI};
use std::sync::Arc;
use tracing::info;
...
pub async fn handle_elo_change_confirmation(
  ...
  if custom_id.starts_with("cancel_elo_change_") {
    ...
    let target_user_id: u64 = custom_id.strip_prefix("cancel_elo_change_").and_then(|s| s.parse().ok()).ok_or_else(|| anyhow::anyhow!("Invalid cancel button ID format"))?;
  ...
  if custom_id.starts_with("confirm_elo_change_") {
    ...
    let parts: Vec<&str> = custom_id.strip_prefix("confirm_elo_change_").ok_or_else(|| anyhow::anyhow!("Invalid confirm button ID format"))?.split('_').collect();
    ...
    // Update in-memory player data and dashboards where this player is queued
    {
      ...
      if found_in_queue {
        info!("Player {} ELO changed, queued {} dashboard update(s)", user_tag, queued);
      } else {
        info!("Player {} ELO changed but not found in any queue", user_tag);
      }
    }
    ...
    let success_embed = CE::new()
      ...
      .description(format!(
        ...
        username, guild_elo.elo, new_elo, old_rank.name, new_rank.name
      ))
      .color(0x00FF00);

    let (embed, components) = nav_player_settings(&settings, db, guild_id).await;

    let response = CIR::UpdateMessage(CIRM::new().embeds(vec![success_embed, embed]).components(components));
    interaction.create_response(&ctx.http, response).await?;
