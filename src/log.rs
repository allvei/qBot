...
pub async fn log_queue_toggle(
  ...
  let pool_size =
    format.sessions.iter().find(|s| s.status == crate::models::SessionStatus::Idle || s.status == crate::models::SessionStatus::Hot).map(|s| (s.pool.len(), format.quota as usize));

  let position = pool_size.as_ref().and_then(|(_current, _)| {
    format
      .sessions
      .iter()
  ...
  log_queue_toggle_sync(&guild_name, &category_name, &player.tag, queue_type, pool_size, Some(&format_name), position);
