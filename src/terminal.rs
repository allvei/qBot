...
pub async fn start_terminal_reader(
            ...
            for format in &mut category.formats {
                for (session_idx, session) in format.sessions.iter_mut().enumerate() {
                    if session.is_idle() {
                        ...
                        println!("[Terminal] Added {} players to session {} in {}/{}/{}", 
                            added, session_idx, guild.id, category.id, format.id);
                        break 'outer;
                    }
                }
            }
            ...
            for format in &mut category.formats {
                for (session_idx, session) in format.sessions.iter_mut().enumerate() {
                    if session.is_idle() {
                        ...
                        if needed == 0 {
                            println!("[Terminal] Session {} already full ({}/{})", 
                                session_idx, session.pool.len(), quota);
                            continue;
                        }
                        ...
                        println!("[Terminal] Filled session {} to quota ({}/{})", 
                            session_idx, session.pool.len(), quota);
                        break 'outer;
                    }
                }
            ...
            for format in &mut category.formats {
                for session in &mut format.sessions {
                    if session.is_idle() && index < session.pool.len() {
                        ...
                        println!("[Terminal] Removed player {} (ELO: {}) at index {}",
                            player.player.tag, player.player.elo, index);
                        removed = true;
                        break 'outer;
                    }
                }
...
async fn cmd_simulate_game(
    _args: &[&str],
    _manager: &Arc<Mutex<Manager>>,
    _db: &Arc<Database>,
) -> anyhow::Result<()> {
            ...
            for format in &category.formats {
                ...
                for (idx, session) in format.sessions.iter().enumerate() {
                    ...
                    for (pidx, player) in session.pool.iter().enumerate() {
                        ...
                        println!("      {}. {}{}{} (ELO: {})", 
                            pidx + 1, vc_indicator, team_str, player.player.tag, player.player.elo);
                    }
                }
            }
        }
...
async fn cmd_resetdb(
    ...
    if let Some(gid) = guild_id {
        ...
        let result = sqlx::query("DELETE FROM guild_settings WHERE guild_id = ?")
            .bind(gid as i64)
            .execute(&db.pool)
            .await;
        ...
        let result = sqlx::query("DELETE FROM player_guild_stats WHERE guild_id = ?")
            .bind(gid as i64)
            .execute(&db.pool)
            .await;
