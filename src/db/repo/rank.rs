...
impl GuildRank {
  ...
  pub fn from_row(row: &SqliteRow, guild_id: GI) -> Option<Self> {
    ...
    Some(GuildRank { guild_id, name, elo: elo as u16, role_id: RoleId::new(role_id as u64) })
  }
}
...
impl RankRepository {
  ...
  pub async fn get_ranks(&self, guild_id: GI) -> Result<Vec<GuildRank>> {
    let rows = sqlx::query("SELECT name, elo, role_id FROM ranks WHERE guild_id = ? ORDER BY elo ASC").bind(guild_id.get() as i64).fetch_all(&self.pool).await?;
  ...
  pub async fn init_default_ranks(&self, guild_id: GI) -> Result<()> {
    ...
    for (name, elo) in defaults {
      sqlx::query("INSERT OR IGNORE INTO ranks (guild_id, name, elo) VALUES (?, ?, ?)").bind(guild_id.get() as i64).bind(&name).bind(elo as i64).execute(&self.pool).await?;
    }
  ...
  pub async fn get_rank_by_name(&self, guild_id: GI, name: &str) -> Result<Option<GuildRank>> {
    let row = sqlx::query("SELECT name, elo, role_id FROM ranks WHERE guild_id = ? AND name = ?").bind(guild_id.get() as i64).bind(name).fetch_optional(&self.pool).await?;
    ...
    match row {
      Some(row) => {
        ...
        Ok(Some(GuildRank { guild_id, name, elo: elo as u16, role_id }))
      }
      None => Ok(None),
    }
  }
  ...
  pub async fn update_rank_name(&self, guild_id: GI, old_name: &str, new_name: &str) -> Result<()> {
    sqlx::query("UPDATE ranks SET name = ? WHERE guild_id = ? AND name = ?").bind(new_name).bind(guild_id.get() as i64).bind(old_name).execute(&self.pool).await?;
    Ok(())
  }

  /// Update a rank's ELO threshold
  pub async fn update_rank_elo(&self, guild_id: GI, name: &str, elo: u16) -> Result<()> {
    sqlx::query("UPDATE ranks SET elo = ? WHERE guild_id = ? AND name = ?").bind(elo as i64).bind(guild_id.get() as i64).bind(name).execute(&self.pool).await?;
    Ok(())
  }
  ...
  pub async fn update_rank_role(&self, guild_id: GI, name: &str, role_id: RoleId) -> Result<()> {
    sqlx::query("UPDATE ranks SET role_id = ? WHERE guild_id = ? AND name = ?").bind(role_id.get() as i64).bind(guild_id.get() as i64).bind(name).execute(&self.pool).await?;
    Ok(())
  }

  /// Delete a rank by name
  pub async fn delete_rank(&self, guild_id: GI, name: &str) -> Result<()> {
    sqlx::query("DELETE FROM ranks WHERE guild_id = ? AND name = ?").bind(guild_id.get() as i64).bind(name).execute(&self.pool).await?;
    Ok(())
  }
