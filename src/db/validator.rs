...
impl DatabaseValidator {
  pub fn new(pool: &SqlitePool) -> Self {
    Self { pool: pool.clone(), migrations: DatabaseMigrations::new(pool), category_repo: CategoryRepository::new(pool.clone()) }
  }
  ...
  async fn check_invalid_user_ids(&self, report: &mut ValidationReport) -> Result<()> {
    let placeholder_categories: i64 = sqlx::query_scalar(
  ...
  async fn check_duplicate_records(&self, report: &mut ValidationReport) -> Result<()> {
    ...
    let duplicate_users: i64 = sqlx::query_scalar("SELECT COUNT(*) - COUNT(DISTINCT user_id) FROM users").fetch_one(&self.pool).await?;
  ...
  async fn validate_configurations(&self, report: &mut ValidationReport) -> Result<()> {
    ...
    let guild_ids: Vec<i64> = sqlx::query_scalar("SELECT DISTINCT guild_id FROM categories").fetch_all(&self.pool).await?;
  ...
  pub async fn repair_database(&self) -> Result<RepairReport> {
    ...
    if placeholder_count > 0 {
      report.manual_actions.push(format!("Found {placeholder_count} categories with placeholder Discord IDs. These require manual configuration.",));
    }
  ...
  async fn remove_duplicate_users(&self) -> Result<i64> {
    let result = sqlx::query("DELETE FROM users WHERE user_id NOT IN (SELECT MIN(user_id) FROM users CATEGORY BY user_id )").execute(&self.pool).await?;
  ...
  async fn count_placeholder_ids(&self) -> Result<i64> {
    let count = sqlx::query_scalar("SELECT COUNT(*) FROM categories WHERE dashboard = 1 OR chat = 1 OR queue = 1").fetch_one(&self.pool).await?;
...
impl ValidationReport {
  fn new() -> Self {
    Self { schema_valid: false, errors: Vec::new(), warnings: Vec::new(), guild_categories: std::collections::HashMap::new() }
  }
...
impl RepairReport {
  fn new() -> Self {
    Self { actions: Vec::new(), manual_actions: Vec::new() }
  }
