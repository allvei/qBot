...
use anyhow::Result;
use pf_pug_bot::{commands, log_prefix_category, RED};
use serenity::all::{
  Client, Command, CommandDataOption, CommandInteraction, CommandOptionType as COT, Context, CreateEmbed, EditMessage, EventHandler, GatewayIntents, Guild, Interaction, Ready,
  VoiceState,
};
...
use pf_pug_bot::{
  ButtonType, CommandContext, ComponentContext, DashboardQueueKey, DashboardUpdateQueue, Database, DmMessageTracker, DmTrackerKey, Manager, Roles, Server, SessionStatus,
  VoiceStateUpdate,
};
...
async fn send_component_error_response(itx: &serenity::all::ComponentInteraction, ctx: &Context, message: &str) {
  ...
  if let Err(e) = itx.create_response(&ctx.http, response).await {
    ...
    if e.to_string().contains("Unknown interaction") {
      debug!("Could not send error response - interaction already handled: {} | User: {} | Message: {}", e, itx.user.id, itx.message.id);
    } else {
      error!(
        "Failed to send error response: {e} | User: {} | Guild: {} | Message: {} | Interaction type: {}",
        itx.user.id,
...
fn is_interaction_valid(interaction: &serenity::all::Interaction) -> bool {
  match interaction {
    serenity::all::Interaction::Component(itx) => itx.id.get() != 0 && itx.message.id.get() != 0,
    serenity::all::Interaction::Command(itx) => itx.id.get() != 0,
    _ => true,
  }
}
    ...
    match pl {
        ...
        let result = match cd.name.as_str() {
          ...
          _ => {
            ...
            match cd.name.as_str() {
              ...
              _ => send_error_response(&itx, &ctx, "Unknown command").await,
            }
          }
        };
      ...
      Interaction::Component(ref itx) => {
        ...
        if itx.data.custom_id.starts_with("category_settings_")
          ...
          || itx.data.custom_id.starts_with("category_sg_")
          ...
          if category.channels.queue_vc == new.channel_id.unwrap() {
            ...
            if let Ok(session) = category.get_user_session(user_id).await {
              ...
              // Now we're guaranteed to have a session
              {
                ...
                let queue_ctx = pf_pug_bot::models::server::QueueContext { ctx: &ctx, guild_id: Some(server), db: Some(&self.db), manager: Some(self.manager.clone()) };
...
impl Handler {
  ...
  async fn handle_player_leave_vc(&self, ctx: &Context, category: &mut pf_pug_bot::models::Category, guild_id: serenity::all::GuildId, user_id: serenity::all::UserId, tag: &str) {
    let guild_name = pf_pug_bot::guild_name(&ctx, guild_id);
...
async fn main() -> Result<()> {
  ...
  let mut client = Client::builder(&token, intents)
    .event_handler(Handler { db: db.clone(), manager: manager.clone(), dashboard_queue: dashboard_queue.clone() })
    .await
  ...
  tokio::spawn(async move {
    ...
    if let Ok(mut manager_lock) = manager_for_shutdown.try_lock() {
      for server in &mut manager_lock.servers {
        ...
        let vc_members: std::collections::HashSet<u64> =
          cache_for_shutdown.guild(guild_id).map(|g| g.voice_states.values().filter_map(|vs| vs.channel_id.map(|c| c.get())).collect()).unwrap_or_default();
    ...
    if let Ok(mut manager_lock) = manager_for_shutdown.try_lock() {
      ...
      for server in &mut manager_lock.servers {
        let guild_id = server.guild_id;
        let guild_name = cache_for_shutdown.guild(guild_id).map(|g| g.name.clone()).unwrap_or_else(|| "Unknown".to_string());
