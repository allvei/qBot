...
impl SetupConfig {
  ...
  pub fn is_complete(&self) -> bool {
    self.dashboard_channel.is_some() && self.queue_channel.is_some() && self.queue_vc_channel.is_some() && self.red_channel.is_some() && self.blue_channel.is_some()
  }
}
...
impl SetupStateManager {
  ...
  pub fn start_setup(&self, user_id: UserId, guild_id: GI) -> SetupConfig {
    ...
    if let Ok(mut states) = self.states.lock() {
      let entry = SetupEntry { config: config.clone(), created_at: Instant::now() };
  ...
  fn cleanup_expired_internal(states: &mut HashMap<(UserId, GI), SetupEntry>) {
    ...
    states.retain(|_, entry| now.duration_since(entry.created_at) < EXPIRY_DURATION);
  }
