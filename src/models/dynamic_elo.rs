...
impl DynamicEloConfig {
    ...
    pub fn calculate_match(
        &self,
        team_red: &[MatchParticipant],
        team_blu: &[MatchParticipant],
        result: MatchResult,
    ) -> Vec<EloChange> {
        ...
        for p in team_red {
            ...
            changes.push(EloChange {
                ...
                new_elo,
                change: delta,
                won: matches!(result, MatchResult::RedWin),
            });
        }
        ...
        for p in team_blu {
            ...
            changes.push(EloChange {
                ...
                new_elo,
                change: delta,
                won: matches!(result, MatchResult::BluWin),
            });
        }
...
pub async fn process_match_elo(
    ...
    for change in &changes {
        if let Err(e) = db.elo.record_dynamic_game(
            change.user_id,
            guild_id,
            change.won,
            change.new_elo,
        ...
        if let Err(e) = db.matches.update_player_elo_after(
            match_id,
            change.user_id,
            change.new_elo as i64,
        ).await {
    ...
    info!("Dynamic ELO applied for match {}: {} players, net drift: {}",
    ...
    if let Ok(avg) = db.elo.get_guild_average_dynamic_elo(guild_id).await {
        ...
        if offset != 0 {
            info!("Normalization triggered: guild dynamic avg={:.1}, anchor={}, applying offset {}",
                avg, config.anchor, offset);
