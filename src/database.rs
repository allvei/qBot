...
impl Database {
    ...
    pub async fn get_group(&self,
        ...
        let group = Group { guild_id:          result.get::<i64, _>("guild") as u64,
                            dashboard:         result.get::<i64, _>("dashboard") as u64,
                            chat:              result.get::<i64, _>("chat") as u64,
                            queue:             result.get::<i64, _>("queue") as u64,
                            teams:             vec![TeamChannels { red: result.get::<i64, _>("red") as u64,
    ...
    pub async fn get_config(&self) -> Result<Config, anyhow::Error> {
        ...
        let config = Config::new(config_map.get("guild_id").unwrap().parse().unwrap(),
