...
fn bch_assign_teams(players: &mut [SessionPlayer]) {
    ...
    for (i, sp) in players.iter_mut().enumerate() {
        sp.team = Some(match (i / 2) % 2 == 0 {
            true  => if i % 2 == 0 { Team::Red } else { Team::Blu },
            false => if i % 2 == 0 { Team::Blu } else { Team::Red },
        });
    }
}
...
pub async fn handle_command(
    ...
    match command {
        ...
        GuiCommand::MovePlayerBetweenSessions { guild_id, category_id, fmt_id, user_id, from_session, to_session } => {
            ...
            let sp = {
                let src = fmt.sessions.get_mut(from_session)
                    .ok_or_else(|| anyhow!("from_session {} out of range", from_session))?;
                let pos = src.pool.iter().position(|p| p.player.user_id == uid)
                    .ok_or_else(|| anyhow!("Player {} not in session {}", user_id, from_session))?;
                src.pool.remove(pos)
            };
            let dst = fmt.sessions.get_mut(to_session)
                .ok_or_else(|| anyhow!("to_session {} out of range", to_session))?;
            dst.pool.push(sp);
        ...
        GuiCommand::ForceTeamRegeneration { guild_id, category_id, fmt_id, session_index } => {
            ...
            let session = fmt.sessions.get_mut(session_index)
                .ok_or_else(|| anyhow!("Session {} not found", session_index))?;
        ...
        GuiCommand::AddPlayer { guild_id, category_id, fmt_id, user_id } => {
            ...
            let fmt = find_format(manager, guild_id, category_id, fmt_id)?;
            let session = fmt.sessions.iter_mut().find(|s| s.is_idle())
                .ok_or_else(|| anyhow!("No idle session to add player to"))?;
            if session.pool.iter().any(|p| p.player.user_id == uid) {
        ...
        GuiCommand::AddDummyPlayers { guild_id, category_id, fmt_id, count, role_id: _ } => {
            let fmt = find_format(manager, guild_id, category_id, fmt_id)?;
            let session = fmt.sessions.iter_mut().find(|s| s.is_idle())
                .ok_or_else(|| anyhow!("No idle session to add dummy players to"))?;
            for i in 0..count {
                let fake_uid = UI::new(900_000_000_000_000_000 + i as u64);
                let elo: u16 = 50 + (i as u16 * 7) % 150;
                let player = Player::add(
                    fake_uid,
                    format!("Dummy{}", i + 1),
                    crate::DEFAULT_QUEUE_EXPIRATION,
                    None,
                    None,
                );
        ...
        GuiCommand::SimulateGameFlow { guild_id, category_id, fmt_id } => {
            ...
            let quota = fmt.quota as usize;
            let session = fmt.sessions.iter_mut().find(|s| s.is_idle())
                .ok_or_else(|| anyhow!("No idle session for SimulateGameFlow"))?;
        ...
        GuiCommand::ViewSessionDetails { guild_id, category_id, fmt_id, session_index } => {
            ...
            match fmt.sessions.get(session_index) {
                Some(s) => {
                    ...
                    for sp in &s.pool {
                        info!("  {} elo={} team={:?} vc={} in_queue={}",
                            sp.player.tag, sp.player.elo, sp.team, sp.in_vc, sp.in_queue);
                    }
                }
