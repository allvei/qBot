...
fn session_block(ui: &mut egui::Ui, session: &crate::models::Session, sess_idx: usize, guild_id: u64, cat_id: u8, fmt_id: u8, state: &GuiSharedState) {
  let blu: Vec<&crate::models::SessionPlayer> = session.pool.iter().filter(|p| p.team == Some(Team::Blu)).collect();
  let red: Vec<&crate::models::SessionPlayer> = session.pool.iter().filter(|p| p.team == Some(Team::Red)).collect();
  ...
  ui.group(|ui| {
    ...
    ui.columns(2, |cols| {
      ...
      for sp in &blu {
        player_row(&mut cols[0], sp, guild_id, cat_id, fmt_id, state);
      }
      cols[1].label(RichText::new("Red").strong());
      for sp in &red {
        player_row(&mut cols[1], sp, guild_id, cat_id, fmt_id, state);
      }
    });
  });
}

fn player_row(ui: &mut egui::Ui, sp: &crate::models::SessionPlayer, guild_id: u64, cat_id: u8, fmt_id: u8, state: &GuiSharedState) {
