...
pub fn show_admin_panel(ui: &mut egui::Ui, state: &GuiSharedState) {
    ...
    let (guild_id, cat_id, fmt_id, session_index) = if let Some(ref m) = manager_opt {
        ...
        let cid = m.qguilds.get(sel_guild)
            .and_then(|g| { sel_cat = sel_cat.min(g.categories.len().saturating_sub(1)); g.categories.get(sel_cat) })
            .map(|c| c.id).unwrap_or(0);
        let fid = m.qguilds.get(sel_guild)
            .and_then(|g| g.categories.get(sel_cat))
            .and_then(|c| { sel_fmt = sel_fmt.min(c.formats.len().saturating_sub(1)); c.formats.get(sel_fmt) })
            .map(|f| f.id).unwrap_or(0);
        let slen = m.qguilds.get(sel_guild)
            .and_then(|g| g.categories.get(sel_cat))
            .and_then(|c| c.formats.get(sel_fmt))
            .map(|f| f.sessions.len()).unwrap_or(1);
    ...
    egui::Grid::new("adm_selectors").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
        if let Some(ref m) = manager_opt {
            // Guild
            ui.label("Guild:");
            egui::ComboBox::from_id_salt("adm_guild_combo")
                .selected_text(m.qguilds.get(sel_guild).map(|g| g.name.as_str()).unwrap_or("—"))
                .show_ui(ui, |ui| {
                    for (i, g) in m.qguilds.iter().enumerate() {
                        if ui.selectable_label(i == sel_guild, &g.name).clicked() {
                            sel_guild = i; sel_cat = 0; sel_fmt = 0; sel_session = 0;
                        }
                    }
                });
            ui.end_row();
            ...
            if let Some(guild) = m.qguilds.get(sel_guild) {
                ui.label("Category:");
                egui::ComboBox::from_id_salt("adm_cat_combo")
                    .selected_text(guild.categories.get(sel_cat)
                        .and_then(|c| c.name.as_deref()).unwrap_or("—"))
                    .show_ui(ui, |ui| {
                        for (i, c) in guild.categories.iter().enumerate() {
                            ...
                            if ui.selectable_label(i == sel_cat, name).clicked() {
                                sel_cat = i; sel_fmt = 0; sel_session = 0;
                            }
                        }
                    });
                ui.end_row();
                ...
                if let Some(cat) = guild.categories.get(sel_cat) {
                    ui.label("Format:");
                    egui::ComboBox::from_id_salt("adm_fmt_combo")
                        .selected_text(cat.formats.get(sel_fmt).map(|f| f.name.as_str()).unwrap_or("—"))
                        .show_ui(ui, |ui| {
                            for (i, f) in cat.formats.iter().enumerate() {
                                if ui.selectable_label(i == sel_fmt, &f.name).clicked() {
                                    sel_fmt = i; sel_session = 0;
                                }
                            }
                        });
                    ui.end_row();
                    ...
                    if let Some(fmt) = cat.formats.get(sel_fmt) {
                        ...
                        egui::ComboBox::from_id_salt("adm_session_combo")
                            .selected_text(format!("Session {} ({:?})",
                                sel_session + 1,
                                fmt.sessions.get(sel_session).map(|s| &s.status).unwrap_or(&SessionStatus::Idle)))
                            .show_ui(ui, |ui| {
            ...
            egui::Grid::new("adm_qm").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    for (label, status) in [
                        ...
                        ("Hot",   SessionStatus::Hot),
                        ("Live",  SessionStatus::Live),
                    ] {
                        if ui.button(label).clicked() {
                            send(GuiCommand::ForceSessionState {
                                guild_id, category_id: cat_id, fmt_id, session_index,
                                new_state: status,
                            });
                        }
                    }
